  ...
  <section id="best-sellers" class="best-sellers py-5 mb-5">
    <div class="container-fluid">
        <div class="d-flex flex-wrap justify-content-between align-items-center mt-5 mb-3">
            <h4 class="text-uppercase">AVAILABLE TOURING VEHICLES</h4>
            <div class="navigation-buttons">
                <a href="{{route('houses')}}" class="prev-button"><i class="fas fa-arrow-left"></i>Go back</a>
                <a href="{{route('other_vehicles')}}" class="next-button">Next <i class="fas fa-arrow-right"></i></a>
            </div>
        </div>

        <div data-aos="fade-in">
            @if($filteredTouringVehicle->count() == 0)
                <div class="alert alert-danger" role="alert">
                    Sorry, all Vehicles in this category are taken.
                </div>
            @else
                <div class="row row-cols-2 row-cols-md-3 row-cols-lg-4">
                    @foreach($filteredTouringVehicle as $select)
                    <div class="col">
...
function openBookingModal(propertyId, status) {
    if (status === 'available') {
        const modalContent = `
            ...
            <br>
            <a style="text-align:right;" class="btn btn-primary" href="{{route('touring_vehicles')}}" class="item-anchor">Cancel</a> 
        `;
