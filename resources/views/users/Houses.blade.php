  ...
  <section id="best-sellers" class="best-sellers py-5 mb-5">
    <div class="container-fluid">
        <div class="d-flex flex-wrap justify-content-between align-items-center mt-5 mb-3">
            <h4 class="text-uppercase">AVAILABLE HOUSES </h4>
            <div class="navigation-buttons">
                <a href="{{route('villas')}}" class="prev-button"><i class="fas fa-arrow-left"></i>Go back</a>
                <a href="{{route('touring_vehicles')}}" class="next-button">Next <i class="fas fa-arrow-right"></i></a>
            </div>
        </div>

        <div data-aos="fade-in">
            @if($filteredHouse->count() == 0)
                <div class="alert alert-danger" role="alert">
                    Sorry, all properties in this category are taken.
                </div>
            @else
                <div class="row row-cols-2 row-cols-md-3 row-cols-lg-4">
                    @foreach($filteredHouse as $select)
                    <div class="col">
...
function openBookingModal(propertyId, status) {
    if (status === 'available') {
        const modalContent = `
            ...
            <br>
            <a style="text-align:right;" class="btn btn-primary" href="{{route('houses')}}" class="item-anchor">Cancel</a> 
        `;
